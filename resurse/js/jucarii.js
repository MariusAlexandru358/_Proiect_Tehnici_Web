window.addEventListener("load",function() {
    ...
    for (let prod of produse){
        var vcul=prod.getElementsByClassName("vculori")[0];
        var listcul=prod.getElementsByTagName("ul")[0];
        let valvcul=vcul.innerHTML.split(",");
        valvcul=valvcul.sort(function (a,b){
                return b.length-a.length;
            });
        for(let cul of valvcul){
            var myli=this.document.createElement("li");
            myli.innerHTML=cul;
            listcul.appendChild(myli);
        }
        vcul.remove();
    ...
    this.document.getElementById("filtrare").onclick=function(){
        ...
        for (let prod of produse){
            ...
            let checkbox = document.getElementsByClassName("pret-examen")[i];
            if(checkbox.checked){
                prod.style.display="grid";
            }
            i++;
    ...
    //     sortareHC(-1);
    // }
    function sortareCustom (semn){
        var produse=document.getElementsByClassName("produs");
        var v_produse= Array.from(produse);
        
        v_produse.sort(function (a,b){

            // var avcul=a.getElementsByClassName("vculori")[0];
            var alistcul=a.getElementsByTagName("li");
            var blistcul=b.getElementsByTagName("li");
            if(alistcul.length==blistcul.length){
                
                let x= semn*(Number(a.getElementsByClassName("val-pret-examen")[0].innerHTML)-Number(b.getElementsByClassName("val-pret-examen")[0].innerHTML))
                return x;
            }
            let x= semn*(alistcul.length-blistcul.length)
            return x;
